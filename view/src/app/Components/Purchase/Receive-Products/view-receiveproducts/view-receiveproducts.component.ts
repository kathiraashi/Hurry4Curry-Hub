  ...
  constructor(
   ...
   ngOnInit() {
      this.active_route.url.subscribe(() => {
         ...
         const Data = { 'User_Id': this.User_Id, HubPurchaseBill_Id: this.SupplierBill_Id };
   ...
   UpdateStock() {
      const Data = { 'User_Id': this.User_Id, 'HubPurchaseBill_Id': this._Data['Supplier_Detail']._id};
      let Info = CryptoJS.AES.encrypt(JSON.stringify(Data), 'SecretKeyIn@123');
      Info = Info.toString();
